...
## 第一步：单张图特征提取指引
请按顺序对每张图片进行简要但定量的特征提取（每张不超过 3 行）。抓取重点如下：
...
### 4. 风险与不确定性提示
结合混合模型与实况的偏差，指出预报的最大不确定性。

### 预测结论
最后，请单独一行给出以下格式的结论（用于程序解析）：
预测结论：降雨概率 XX%，预测等级 X
...
def analyze_images(
    ...
    image_paths 可以是本地文件路径（Path）或远程 URL（字符串）。
    """
    client = OpenAI(
